...
Para integração em pipelines Python, o padrão recomendado é carregar o `SKILL.md` e apenas os arquivos de referência relevantes para o artefato em questão — evitando carregar os 34 arquivos a cada chamada.
...
### Carregamento seletivo das referências
...
# Arquivos comuns são sempre incluídos (qualidade + gramática)
COMUNS = ["op_qualidade.md", "mot_qualidade.md", "op_apendice_gramatica.md"]

REFERENCIAS = {
    "introducao":        ["op_introducao.md", "mot_estrutura_relatorio.md"],
    ...
    "redacao":           ["op_apendice_paragrafos.md"],
...
def carregar_referencias(tipo_artefato: str) -> str:
    arquivos = COMUNS + REFERENCIAS.get(tipo_artefato, [])
    blocos = []
    for arquivo in arquivos:
        pasta = "references" if arquivo != "SKILL.md" else ""
        conteudo = httpx.get(f"{BASE}/references/{arquivo}").text
...
tipo_artefato = "introducao"  # ver chaves do dicionário REFERENCIAS
artefato = "<texto do artefato a ser analisado>"
...
```python
tipo_artefato = "achado"  # artefato a ser gerado
insumos = "<matriz de achados ou dados estruturados>"

response = client.chat.completions.create(
    model="<nome-do-deployment>",
    messages=[
        {"role": "system", "content": montar_system_prompt(tipo_artefato)},
        {"role": "user", "content": f"Gere um {tipo_artefato} a partir dos seguintes insumos:\n\n{insumos}"},
    ],
)
print(response.choices[0].message.content)
```
